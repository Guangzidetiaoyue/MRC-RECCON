...
from transformers import RobertaPreTrainedModel, RobertaModel, BertModel,BertPreTrainedModel, BertForSequenceClassification,BertForTokenClassification
...
class RobertaForQuestionAnswering(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        all_answer_cls:Optional[torch.LongTensor] =None
        ...
        if all_answer_cls is None:
            logits = self.qa_outputs(sequence_output_0) # sequence_output_0
            ...
            logits = self.classifier(sequence_output_0)
            outputs = (logits,) + outputs[2:]
            if all_answer_cls is not None:
                if self.num_labels == 1:
                    #  We are doing regression
                    loss_fct = MSELoss()
                    loss = loss_fct(logits.view(-1), all_answer_cls.view(-1))
                else:
                    loss_fct = CrossEntropyLoss()
                    loss = loss_fct(logits.view(-1, self.num_labels), all_answer_cls.view(-1))
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        if all_answer_cls is None:
            ...
            # sequence_output_1 = self.dropout(sequence_output_1)
